...
```bash
# 获取所有待执行任务
curl "http://localhost:6767/api/v1/tasks?status=pending"

# 获取特定组的任务
curl "http://localhost:6767/api/v1/tasks?group=docker"
```
...
**请求示例**:
```bash
curl "http://localhost:6767/api/v1/tasks/pending?group=docker"
```
...
**请求示例**:
```bash
curl -X POST http://localhost:6767/api/v1/workers/worker-001/heartbeat
```
...
**请求示例**:
```bash
curl "http://localhost:6767/api/v1/workers?group=linux"
```
...
**请求示例**:
```bash
curl http://localhost:6767/api/v1/stats/groups
```
...
# 锁定任务
curl -X POST http://localhost:6767/api/v1/tasks/1/lock \
  -H "Content-Type: application/json" \
  -d '{"worker_id": "worker-001"}'

# 完成任务
curl -X POST http://localhost:6767/api/v1/tasks/1/complete \
  -H "Content-Type: application/json" \
