...
Master 服务将在 `http://localhost:6767` 启动。
...
# 连接到远程 Master
cpds worker --master http://master.example.com:6767
...
```bash
# Master 服务器
cpds master --db /data/agentflow.db --host 0.0.0.0 --port 6767
...
# 添加任务
curl -X POST http://master-server:6767/api/v1/tasks \
...
# 启动 Worker
docker run -d \
  -e MASTER_URL=http://cpds-master:6767 \
...
检查以下几点：
1. Master 服务是否正常运行：`curl http://localhost:6767/health`
