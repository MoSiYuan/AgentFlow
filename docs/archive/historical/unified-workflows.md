# AgentFlow 统一流程文档

## 📋 文档说明

本文档定义 AgentFlow 的核心业务流程，确保所有语言版本（Python/Go/Node.js）行为一致。

---

## 🎯 核心流程总览

```
1. 系统启动流程
2. 任务创建流程
3. 任务执行流程
4. Git 集成流程
5. 冲突解决流程
6. Worker 生命周期
7. 错误处理流程
8. 关闭与清理流程
```

---

## 1️⃣ 系统启动流程

### Master 启动流程

```
┌─────────────────────────────────────────────────────────────┐
│                     Master 启动                             │
└─────────────────────────────────────────────────────────────┘

1. 加载配置
   ├─ 读取配置文件 (master.config.yaml)
   ├─ 应用环境变量覆盖
   └─ 验证配置有效性

2. 初始化数据库
   ├─ 打开/创建数据库文件
   ├─ 执行 migrations（如需要）
   ├─ 初始化索引和视图
   └─ 验证数据库完整性

3. 初始化组件
   ├─ 创建 HTTP 服务器
   ├─ 设置路由和中间件
   ├─ 初始化 WebSocket 服务器（如启用）
   ├─ 创建事件总线
   └─ 初始化 Git 集成（如启用）

4. 启动后台服务
   ├─ 心跳清理线程（每 60 秒）
   │  └─ 清理超时 Worker
   ├─ 统计信息收集（每 30 秒）
   └─ 自动关闭监控（如启用）

5. 启动本地 Workers（如配置）
   ├─ 检测系统类型（darwin/linux/windows）
   ├─ 为每个组启动一个 Worker
   └─ Worker 进程管理

6. 开始监听
   ├─ 绑定 HTTP 端口 (默认 6767)
   ├─ 绑定 WebSocket 端口 (默认 8849)
   └─ 输出启动信息

→ Master Ready
```

### Worker 启动流程

```
┌─────────────────────────────────────────────────────────────┐
│                     Worker 启动                             │
└─────────────────────────────────────────────────────────────┘

1. 加载配置
   ├─ 读取配置文件 (worker.config.yaml)
   ├─ 应用环境变量覆盖
   └─ 验证配置有效性

2. 生成 Worker ID（如未指定）
   ├─ 格式: worker-{hostname}-{pid}-{random}
   └─ 或使用 UUID

3. 初始化组件
   ├─ HTTP 客户端（用于 Master 通信）
   ├─ Claude 执行器（检测可用性）
   ├─ Shell 执行器
   └─ 工作空间管理器

4. 注册到 Master
   ├─ POST /api/v1/workers/register
   ├─ { worker_id, group_name, platform, capabilities }
   └─ → Worker registered

5. 启动心跳
   ├─ 定时器：每 30 秒
   ├─ POST /api/v1/workers/:id/heartbeat
   └─ 失败 3 次标记为离线

6. 进入任务循环
   ├─ 根据 mode 决定行为
   │  ├─ auto: 自动拉取和执行任务
   │  ├─ manual: 等待手动分配
   │  └─ oneshot: 执行一个任务后退出
   └─ 开始任务拉取

→ Worker Ready
```

---

## 2️⃣ 任务创建流程

### API 任务创建

```
┌─────────────────────────────────────────────────────────────┐
│                   任务创建 (API)                             │
└─────────────────────────────────────────────────────────────┘

Client                           Master
  │                                │
  ├─ POST /api/v1/tasks ─────────>│
  │  {                              │
  │    title: "实现用户登录",        │
  │    description: "...",          │
  │    group_name: "frontend",      │
  │    priority: "high"             │
  │  }                              │
  │                                │
  │                                ├─ 1. 验证输入
  │                                │  ├─ 必填字段检查
  │                                │  └─ 字段类型验证
  │                                │
  │                                ├─ 2. 生成任务 ID
  │                                │  └─ TASK-XXXXXXXX
  │                                │
  │                                ├─ 3. 创建工作空间（如需要）
  │                                │  └─ .agentflow/workspace/TASK-XXX/
  │                                │
  │                                ├─ 4. 插入数据库
  │                                │  INSERT INTO tasks (...)
  │                                │
  │                                ├─ 5. 广播事件
  │                                │  event: 'task.created'
  │                                │
  │<──────── 201 Created ──────────┤
  │  {                              │
  │    task_id: "TASK-XXX",         │
  │    status: "pending",           │
  │    created_at: "2026-01-22..."  │
  │  }                              │
  │                                │
→ Task Created                     │
```

### 父子任务创建

```
┌─────────────────────────────────────────────────────────────┐
│                 父子任务创建                                 │
└─────────────────────────────────────────────────────────────┘

1. 创建父任务
   POST /api/v1/tasks
   { title: "开发用户管理功能" }
   → TASK-001 (status: pending)

2. 分解为子任务
   POST /api/v1/tasks
   {
     parent_id: "TASK-001",
     title: "实现登录 API",
     description: "..."
   }
   → TASK-002 (parent_id: TASK-001)

   POST /api/v1/tasks
   {
     parent_id: "TASK-001",
     title: "实现注册 API",
     description: "..."
   }
   → TASK-003 (parent_id: TASK-001)

3. 任务树结构
   TASK-001 (父任务)
   ├─ TASK-002 (子任务)
   └─ TASK-003 (子任务)

4. 执行逻辑
   - 子任务可以独立执行
   - 父任务等待所有子任务完成
   - 父任务状态 = f(子任务状态)
```

---

## 3️⃣ 任务执行流程

### 标准任务执行

```
┌─────────────────────────────────────────────────────────────┐
│                   任务执行流程                               │
└─────────────────────────────────────────────────────────────┘

Worker                           Master                    Database
  │                                │                           │
  ├─ 1. 拉取待执行任务 ──────────>│                           │
  │  GET /api/v1/tasks/pending?   │                           │
  │      group=frontend           │                           │
  │                                │                           │
  │                                ├─ 查询数据库 ─────────────>│
  │                                │ SELECT * FROM tasks       │
  │                                │ WHERE status='pending'    │
  │                                │ AND group_name='frontend'│
  │                                │                           │
  │                                │<─ 返回任务列表 ───────────┤
  │                                │                           │
  │<──────── 返回任务列表 ─────────┤                           │
  │  [TASK-001, TASK-002, ...]    │                           │
  │                                │                           │
  ├─ 2. 选择任务                   │                           │
  │  (按优先级排序)                │                           │
  │  → TASK-001                    │                           │
  │                                │                           │
  ├─ 3. 尝试锁定任务 ────────────>│                           │
  │  POST /api/v1/tasks/TASK-001/ │                           │
  │       lock                     │                           │
  │  { worker_id: "worker-1" }     │                           │
  │                                │                           │
  │                                ├─ UPDATE tasks ───────────>│
  │                                │ SET status='running',     │
  │                                │ lock_holder='worker-1',   │
  │                                │ lock_time=NOW()           │
  │                                │ WHERE id='TASK-001'       │
  │                                │                           │
  │<──────── 锁定成功 ─────────────┤                           │
  │  { status: "locked" }          │                           │
  │                                │                           │
  ├─ 4. 执行任务                   │                           │
  │  ├─ 尝试优先级 1: HTTP Executor│                           │
  │  │  ├─ 检查 Claude Server      │                           │
  │  │  ├─ 如可用，发送请求        │                           │
  │  │  └─ → 成功则跳过后续        │                           │
  │  │                             │                           │
  │  ├─ 尝试优先级 2: Claude CLI   │                           │
  │  │  ├─ 检查 claude 命令        │                           │
  │  │  ├─ 执行 claude prompt      │                           │
  │  │  └─ → 成功则跳过后续        │                           │
  │  │                             │                           │
  │  └─ 尝试优先级 3: Shell Command│                           │
  │     ├─ 解析命令前缀            │                           │
  │     │  ├─ "shell:" → 直接执行  │                           │
  │     │  └─ 无前缀 → Shell 解释  │                           │
  │     ├─ 执行命令                │                           │
  │     └─ 捕获输出                │                           │
  │                                │                           │
  ├─ 5. 上报进度（可选）           │                           │
  │  POST /api/v1/tasks/TASK-001/ │                           │
  │       progress                 │                           │
  │  { progress: 50,               │                           │
  │    message: "Processing..." }  │                           │
  │                                │                           │
  │                                ├─ 广播进度事件 ────────────>│
  │                                │ event: 'task.progress'    │
  │                                │                           │
  ├─ 6. 完成任务 ────────────────>│                           │
  │  POST /api/v1/tasks/TASK-001/ │                           │
  │       complete                 │                           │
  │  { worker_id: "worker-1",      │                           │
  │    result: "..." }             │                           │
  │                                │                           │
  │                                ├─ UPDATE tasks ───────────>│
  │                                │ SET status='completed',   │
  │                                │ result='...',             │
  │                                │ completed_at=NOW()        │
  │                                │ lock_holder=NULL         │
  │                                │                           │
  │                                ├─ 广播完成事件 ───────────>│
  │                                │ event: 'task.completed'   │
  │                                │                           │
  │<──────── 200 OK ──────────────┤                           │
  │                                │                           │
→ Task Completed                   │                           │
```

### 执行器优先级流程

```
┌─────────────────────────────────────────────────────────────┐
│              三级执行器选择流程                               │
└─────────────────────────────────────────────────────────────┘

function executeTask(task):
    result = null
    error = null

    # 优先级 1: HTTP Executor (Claude Server)
    if httpExecutor.isAvailable():
        try:
            result = httpExecutor.execute(task)
            if result.success:
                return result
        except Exception as e:
            error = e
            log.warn("HTTP Executor failed:", e)

    # 优先级 2: Claude CLI Executor
    if claudeExecutor.isEnabled():
        try:
            result = claudeExecutor.execute(task)
            if result.success:
                return result
        except Exception as e:
            error = e
            log.warn("Claude CLI failed:", e)

    # 优先级 3: Shell Command Executor
    try:
        result = shellExecutor.execute(task)
        return result
    except Exception as e:
        log.error("All executors failed")
        raise TaskExecutionError("All executors failed", error)

# 命令解析规则
function parseCommand(description):
    if description.startsWith("shell:"):
        # 显式 shell 命令
        cmd = description.substring(6)
        return { type: "shell", command: cmd }

    elif description.startsWith("http:"):
        # 显式 HTTP 请求
        url = description.substring(5)
        return { type: "http", url: url }

    elif description.startsWith("claude:"):
        # 显式 Claude CLI
        prompt = description.substring(7)
        return { type: "claude", prompt: prompt }

    else:
        # 自动检测
        if containsCodeBlocks(description):
            return { type: "claude", prompt: description }
        else:
            return { type: "shell", command: description }
```

---

## 4️⃣ Git 集成流程

### Agent 任务与 Git 分支

```
┌─────────────────────────────────────────────────────────────┐
│              Git 集成任务执行流程                             │
└─────────────────────────────────────────────────────────────┘

1. 创建 Git 任务
   ├─ POST /api/v1/git/tasks
   │  {
   │    title: "实现用户认证",
   │    agent_id: "agent-frontend",
   │    file_boundaries: [...]
   │  }
   ├→ GitTask {
        id: "GT-001",
        git_branch: "agent-agent-frontend/task-GT-001"
      }

2. 创建 Git 分支
   ├─ git checkout -b agent-agent-frontend/task-GT-001
   ├→ 分支创建成功

3. 验证文件访问权限
   ├─ 读取 .agentflow/boundaries.json
   ├─ 查找 agent-frontend 的边界规则
   ├─ 检查请求的文件是否在允许范围内
   │  ├─ access_type = "exclusive" → 只有该 agent 可访问
   │  ├─ access_type = "shared" → 多个 agent 可访问（需锁）
   │  └─ access_type = "readonly" → 只读访问
   └→ Allowed / Denied

4. 获取文件锁（如需要）
   ├─ 检查文件是否已锁定
   │  SELECT * FROM git_locks
   │  WHERE file_path=? AND status='active'
   ├─ 如未锁定，创建锁
   │  INSERT INTO git_locks (...)
   └→ Lock acquired / Conflict

5. Agent 在分支上工作
   ├─ 修改文件
   ├─ git add .
   ├─ git commit -m "agent-frontend: ..."
   └→ 提交在独立分支

6. 尝试合并到主分支
   ├─ git checkout main
   ├─ git merge --squash agent-agent-frontend/task-GT-001
   ├─ 检测冲突
   │  ├─ 无冲突 → 合并成功
   │  └─ 有冲突 → 触发升级机制
   └→ Success / Conflict
```

### 冲突解决流程

```
┌─────────────────────────────────────────────────────────────┐
│              三级冲突解决流程                                 │
└─────────────────────────────────────────────────────────────┘

检测到冲突
    │
    ▼
┌───────────────────┐
│ 级别 1: 自动重试   │
├───────────────────┤
│ • 等待 2 秒        │
│ • 重新尝试合并     │
│ • 最多 3 次       │
└─────────┬─────────┘
          │
          ├─ 成功 → 任务完成 ✅
          │
          └─ 仍然失败
             │
             ▼
┌───────────────────┐
│ 级别 2: 升级任务   │
├───────────────────┤
│ • 创建新任务       │
│   UPGRADE-{id}    │
│ • 更高优先级       │
│ • 包含冲突信息     │
│ • 自动解决尝试     │
└─────────┬─────────┘
          │
          ├─ 可自动解决
          │  └─ 分析冲突
          │     ├─ 修改不重叠 → 自动合并
          │     └─ 合并成功 → 任务完成 ✅
          │
          └─ 需要人工解决
             │
             ▼
┌───────────────────┐
│ 级别 3: 人工介入   │
├───────────────────┤
│ • 标记状态为       │
│   "manual"        │
│ • 通知相关人员     │
│ • 等待人工解决     │
│ • 验证解决结果     │
└───────────────────┘
          │
          └─ 人工解决完成 → 任务完成 ✅
```

---

## 5️⃣ Worker 生命周期

### Worker 注册与心跳

```
┌─────────────────────────────────────────────────────────────┐
│              Worker 生命周期                                 │
└─────────────────────────────────────────────────────────────┘

1. 启动
   ├─ 生成/加载 Worker ID
   ├─ 检测系统平台和组
   └─ 初始化执行器

2. 注册到 Master
   ├─ POST /api/v1/workers/register
   │  {
   │    worker_id: "worker-1",
   │    group_name: "darwin",
   │    platform: "darwin",
   │    capabilities: ["claude", "gui", "cross-platform"]
   │  }
   ├→ Worker registered
   └→ INSERT INTO workers (...)

3. 心跳循环
   ├─ 每 30 秒发送心跳
   │  POST /api/v1/workers/worker-1/heartbeat
   ├→ UPDATE workers SET last_heartbeat=NOW()
   ├─ 连续失败 3 次 → 标记为离线
   └→ 重新尝试连接

4. 任务执行循环
   ├─ GET /api/v1/tasks/pending?group=darwin
   ├─ 如有任务
   │  ├─ POST /api/v1/tasks/:id/lock
   │  ├─ 执行任务
   │  ├─ POST /api/v1/tasks/:id/complete
   │  └─ 继续拉取
   └─ 如无任务
      └─ 等待 5 秒后重试

5. 正常关闭
   ├─ 收到关闭信号
   │  ├─ 完成当前任务
   │  ├─ 取消任务锁定
   │  └─ 通知 Master
   └→ Worker offline

6. 异常关闭
   ├─ 进程崩溃
   ├─ Master 检测到心跳超时（120 秒）
   └→ UPDATE workers SET status='inactive'
```

---

## 6️⃣ 错误处理流程

### 任务执行失败

```
┌─────────────────────────────────────────────────────────────┐
│              任务失败处理                                    │
└─────────────────────────────────────────────────────────────┘

1. 检测失败
   ├─ 执行器抛出异常
   ├─ 超时（默认 600 秒）
   └─ 内存超限

2. 记录错误
   ├─ INSERT INTO task_logs
   │  (task_id, worker_id, log_level='error', message)
   └→ Error logged

3. 更新任务状态
   ├─ POST /api/v1/tasks/:id/fail
   │  { worker_id, error }
   ├→ UPDATE tasks
   │  SET status='failed',
   │      error='...',
   │      completed_at=NOW()
   └→ Task failed

4. 错误处理策略
   ├─ 如果 retry_on_failure=true
   │  └─ 重置为 pending，允许重试
   ├─ 如果超过 max_retries
   │  └─ 保持 failed 状态
   └─ 发送失败事件
      event: 'task.failed'

5. 依赖任务处理
   ├─ 如果有子任务
   │  └─ 取消所有 pending 子任务
   └─ 如果是子任务
      └─ 通知父任务
```

### Master 错误处理

```
┌─────────────────────────────────────────────────────────────┐
│              Master 错误处理                                 │
└─────────────────────────────────────────────────────────────┘

1. 数据库连接失败
   ├─ 重试连接（最多 3 次）
   ├─ 如仍失败
   │  └─ FATAL: 无法启动 Master
   └→ 退出并记录错误

2. HTTP 服务器启动失败
   ├─ 检查端口占用
   ├─ 尝试其他端口
   └─ 如全部失败 → FATAL

3. Worker 心跳超时
   ├─ 标记 Worker 为离线
   ├─ 释放该 Worker 的任务锁
   │  UPDATE tasks
   │  SET lock_holder=NULL
   │  WHERE lock_holder=?
   └→ 任务可被其他 Worker 执行

4. 内存不足
   ├─ 记录警告
   ├─ 拒绝新任务
   └─ 等待资源释放
```

---

## 7️⃣ 关闭与清理流程

### 优雅关闭

```
┌─────────────────────────────────────────────────────────────┐
│              优雅关闭流程                                    │
└─────────────────────────────────────────────────────────────┘

Master 关闭:
    │
    ├─ 1. 停止接受新任务
    │  └─ POST /api/v1/tasks 返回 503
    │
    ├─ 2. 等待运行中的任务完成
    │  ├─ 检查 status='running' 的任务
    │  └─ 最多等待 300 秒
    │
    ├─ 3. 通知所有 Workers
    │  ├─ WebSocket broadcast: shutdown event
    │  └─ Workers 优雅退出
    │
    ├─ 4. 停止后台服务
    │  ├─ 心跳清理线程
    │  ├─ 统计收集线程
    │  └─ 事件监听器
    │
    ├─ 5. 关闭数据库连接
    │  └─ db.close()
    │
    └─ 6. 退出
       └→ exit(0)

Worker 关闭:
    │
    ├─ 1. 完成当前任务
    │  ├─ 等待任务执行完成
    │  └─ 上报结果
    │
    ├─ 2. 释放任务锁
    │  └─ POST /api/v1/tasks/:id/unlock
    │
    ├─ 3. 通知 Master
    │  └─ 最后一次心跳
    │
    └─ 4. 退出
       └→ exit(0)
```

### 强制关闭

```
┌─────────────────────────────────────────────────────────────┐
│              强制关闭（SIGKILL）                              │
└─────────────────────────────────────────────────────────────┘

1. 接收到 SIGKILL 信号
    │
    ├─ 无法优雅关闭
    │  └─ 进程立即终止
    │
    ├─ 数据库状态
    │  ├─ 任务锁保持（lock_holder 仍设置）
    │  └─ 下次启动时清理
    │
    └→ 自动清理机制
        ├─ Master 重启时
        │  └─ 清理超时的任务锁
        └─ Worker 重连时
           └─ 检测并释放旧锁
```

---

## 8️⃣ 完整示例：端到端任务流

```
┌─────────────────────────────────────────────────────────────┐
│           完整任务执行示例（时间线）                          │
└─────────────────────────────────────────────────────────────┘

T0: 客户端创建任务
    POST /api/v1/tasks
    { title: "重构用户模块", group_name: "frontend" }
    → TASK-001 created

T1: Worker-1 拉取任务
    GET /api/v1/tasks/pending?group=frontend
    → [TASK-001]

T2: Worker-1 锁定任务
    POST /api/v1/tasks/TASK-001/lock
    { worker_id: "worker-1" }
    → locked

T3: Worker-1 尝试 HTTP Executor
    → Claude Server 不可用，跳过

T4: Worker-1 尝试 Claude CLI
    $ claude "重构用户模块..."
    → 执行中...

T5: Worker-1 上报进度
    POST /api/v1/tasks/TASK-001/progress
    { progress: 30, message: "正在分析代码..." }
    → progress updated

T6: Worker-1 上报进度
    POST /api/v1/tasks/TASK-001/progress
    { progress: 60, message: "正在重构..." }
    → progress updated

T7: Worker-1 完成任务
    POST /api/v1/tasks/TASK-001/complete
    { worker_id: "worker-1", result: "重构完成，修改了 5 个文件" }
    → completed

T8: Master 广播事件
    WebSocket: { type: 'task.completed', data: {...} }
    → 所有监听者收到通知

→ 任务生命周期结束
```

---

## 🎯 流程图符号说明

```
┌─────────┐  开始/结束
│  Start  │
└─────────┘

┌─────────┐  处理步骤
│ Process │
└─────────┘

┌─────────┐  判断
│ Decision │
└─────────┘

    ──→     顺序流程
    ─┼→     并行流程
   <--->    双向通信
```

---

**版本**: v1.0.0
**更新**: 2026-01-22
**作者**: AgentFlow Team
**相关文档**: [统一架构设计](./unified-architecture.md)
